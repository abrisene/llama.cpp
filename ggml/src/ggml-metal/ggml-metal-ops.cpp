...
#undef FATTN_SMEM
        ...
        //
#define FATTN_SMEM(nsg) (GGML_PAD(((GGML_PAD(ne00, 128) + 4*ncpsg + 2*GGML_PAD(ne20, 128))*(nsg)*nqptg*nhptg)*(sizeof(float)/2), 16))
        ...
        auto pipeline = ggml_metal_library_get_pipeline_flash_attn_ext_vec(
            lib, op, has_mask, has_sinks, has_bias, has_scap, has_kvpad,
            nqptg, cfg.NE, nsg, nwg, nhptg, use_q8_f16);
