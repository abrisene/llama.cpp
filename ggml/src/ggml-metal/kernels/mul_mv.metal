...
void kernel_mul_mv_tq2_0_f32_impl(
    ...
    constexpr short NB = N_SIMDWIDTH/NBLOCK;

    const short blk = tiisg / NB;
    const short htg = tiisg % NB;
        ...
        FOR_UNROLL (short h0 = 0; h0 < 2; ++h0) {
            ...
            FOR_UNROLL (short row = 0; row < nr0; ++row) {
                ...
                FOR_UNROLL (short j = 0; j < 4; ++j) {
                    ...
                    const float f0 = v;
                    const float f1 = floor(v*0.25f);
                    const float f2 = floor(v*0.0625f);
                    const float f3 = floor(v*0.015625f);
...
kernel void kernel_mul_mv_tq2_0_f32(
        ...
        ushort tiisg[[thread_index_in_simdgroup]],
        ushort sgitg[[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_tq2_0_f32_impl<N_R0_TQ2_0, constant ggml_metal_kargs_mul_mv &>(
